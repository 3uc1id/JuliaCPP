...
void julia_worker(JuliaParams *params, uint64_t row_step, uint64_t row_start){
    ...
    Technically, a better looking picture could be generated by sampling multiple points
    in each cell, then computing the luminance weighted average of the colors. If I
    get around to it I will implement that. I'm not sure if randomly selected points
    would work better, or if the cell should be split up into sectors with and their
    centroids sampled, or some other scheme.

    */

    double real_start = params->top_left.real();
    double real_end = params->bottom_right.real();
    double real_cell_width = (real_end - real_start)/params->img.getWidth();
    double imag_cell_height = (params->top_left.imag() - params->bottom_right.imag())/params->img.getHeight();
    ...
    */
    double i_part = params->top_left.imag() - imag_cell_height/2.0 - row_start*imag_cell_height;
    Color interior; // default is black
    for(uint64_t row=row_start; row < params->img.getHeight(); row+=row_step){
        double real_part = real_start + real_cell_width/2.0;
        for(uint64_t col=0; col < params->img.getWidth(); col++){
            ...
            real_part += real_cell_width; // step right towards positive real
        }
        i_part -= imag_cell_height*row_step; // step down towards negative imaginary
    }
}
